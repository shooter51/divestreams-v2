...
<body style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; line-height: 1.6; color: #333; max-width: 600px; margin: 0 auto; padding: 20px;">
  <div style="background: linear-gradient(135deg, #0066cc 0%, #0099ff 100%); padding: 30px; border-radius: 10px 10px 0 0; text-align: center;">
    ...
    <p style="color: rgba(255,255,255,0.9); margin: 10px 0 0 0; font-size: 14px;">${data.businessName}</p>
  </div>
    ...
    <div style="background: white; border: 1px solid #e5e7eb; border-radius: 8px; padding: 20px; margin: 20px 0;">
      <div style="display: flex; justify-content: space-between; margin-bottom: 15px; padding-bottom: 15px; border-bottom: 2px solid #e5e7eb;">
        <div>
          ...
          <p style="margin: 5px 0 0 0; font-weight: bold; font-size: 14px;">${data.receiptNumber}</p>
        </div>
        <div style="text-align: right;">
          <p style="margin: 0; color: #666; font-size: 12px;">DATE</p>
          <p style="margin: 5px 0 0 0; font-size: 14px;">${data.transactionDate}</p>
        </div>
      </div>
        ...
        ${data.items.map(item => `
          <tr>
            <td style="padding: 8px 0; border-bottom: 1px solid #f3f4f6;">
              ...
              ${item.quantity ? `<div style="font-size: 12px; color: #666;">${item.quantity} × ${data.currency.toUpperCase()} ${item.unitPrice.toFixed(2)}</div>` : ''}
            </td>
            <td style="padding: 8px 0; text-align: right; border-bottom: 1px solid #f3f4f6; font-weight: 500;">
              ${data.currency.toUpperCase()} ${item.total.toFixed(2)}
            </td>
          </tr>
        `).join('')}
      </table>
      ...
      <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
        <tr>
          ...
          <td style="padding: 8px 0; text-align: right;">${data.currency.toUpperCase()} ${data.subtotal.toFixed(2)}</td>
        </tr>
        <tr>
          <td style="padding: 8px 0; color: #666;">${data.taxName}:</td>
          <td style="padding: 8px 0; text-align: right;">${data.currency.toUpperCase()} ${data.tax.toFixed(2)}</td>
        </tr>
        <tr style="border-top: 2px solid #e5e7eb;">
          ...
          <td style="padding: 12px 0; text-align: right; font-weight: bold; font-size: 16px; color: #059669;">
            ${data.currency.toUpperCase()} ${data.total.toFixed(2)}
          </td>
        </tr>
        <tr>
          <td style="padding: 8px 0; color: #666;">Payment Method:</td>
          <td style="padding: 8px 0; text-align: right; text-transform: capitalize;">${data.paymentMethod}</td>
        </tr>
      </table>
    </div>
...
ITEMS:
${data.items.map(item => `- ${item.name}${item.quantity ? ` (${item.quantity} × ${data.currency.toUpperCase()} ${item.unitPrice.toFixed(2)})` : ''}: ${data.currency.toUpperCase()} ${item.total.toFixed(2)}`).join('\n')}

SUMMARY:
Subtotal: ${data.currency.toUpperCase()} ${data.subtotal.toFixed(2)}
${data.taxName}: ${data.currency.toUpperCase()} ${data.tax.toFixed(2)}
Total: ${data.currency.toUpperCase()} ${data.total.toFixed(2)}
