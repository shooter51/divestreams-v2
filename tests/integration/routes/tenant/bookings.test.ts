...
/**
 * Sets up db.select mock for the loader's 3 queries:
 * 1. bookingList  (select/from/leftJoin×3/where/orderBy/limit/offset → rows)
 * 2. totalCount   (select/from/where → [{ value }])
 * 3. monthlyCount (select/from/where → [{ value }])
 *
 * Stats (today, upcoming, pendingPayment) are computed from the booking rows,
 * not from separate DB queries.
 */
function setupDbMock(opts: {
  ...
  const { bookings: rows = [], total = 0, monthly = 0 } = opts;
  ...
  const queries = [
    ...
    countQuery(total),  // 2. total count
    countQuery(monthly),// 3. monthly count
  ];

  let callCount = 0;
  (db.select as Mock).mockImplementation(() => {
    const q = queries[callCount] ?? countQuery(0);
    callCount++;
    return q;
  });
      ...
      const mockRows = [
        {
          ...
          tripDate: new Date("2025-06-15"),
          tripTime: "14:00",
          participants: 1,
          total: 100.0,
          status: "confirmed",
          paidAmount: 50.0, // partially paid — triggers pendingPayment
        ...
        },
        {
          ...
          total: 100.0,
          status: "confirmed",
          paidAmount: 100.0, // fully paid
          createdAt: new Date("2025-01-10"),
        },
      ];
      setupDbMock({ bookings: mockRows, total: 2, monthly: 2 });
      const result = await loader(makeRequest());
