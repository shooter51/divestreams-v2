 ...
 *
 * This route handles the OAuth callback from Xero after the user
 ...
 *
 * Route: /api/integrations/xero/callback
 */
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  // Handle OAuth errors
  if (error) {
    integrationLogger.error({ err: error, provider: "xero" }, "Xero OAuth error");
    const errorMessage = encodeURIComponent(
      error === "access_denied"
        ? "You declined the Xero access request."
        : `Xero authorization failed: ${errorDescription || error}`
    );
    return redirect(
  ...
  try {
    ...
    await handleXeroCallback(code, orgId, subdomain || undefined);

    // Redirect back to integrations page with success message
    return redirect(
      getRedirectUrl(
        "/tenant/settings/integrations?success=" +
          encodeURIComponent("Xero connected successfully!")
      )
    );
  } catch (err) {
    integrationLogger.error({ err, provider: "xero" }, "OAuth callback failed");
    const errorMessage =
  ...
  return (
    <div className="flex items-center justify-center min-h-screen">
      <div className="text-center">
        ...
        <p className="text-foreground-muted">Connecting Xero...</p>
      </div>
    </div>
  );
}
